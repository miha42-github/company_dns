...
def download_eu_sic_data(config):
    ...
    """
    # Get the configuration values
    download_site = config['sic_data']['EU_SIC_DATA_SITE']
    ...
    # Check if any NACE files already exist
    existing_files = glob.glob(os.path.join(data_dir, 'NACE_Rev*_Heading_All_Languages.tsv'))
    if existing_files:
        latest_file = max(existing_files)
        logger.info('Found existing EU SIC data file: %s', latest_file)
        return latest_file
    
    # Download the page and find the download link
    logger.info('Checking for EU SIC data at %s', download_site)
    try:
        response = requests.get(download_site)
        response.raise_for_status()  # Raise exception for HTTP errors
        ...
        # Look for links containing the TSV file
        for link in soup.find_all('a', href=True):
            href = link['href']
            text = link.get_text()
            # Look for the specific TSV file with some flexibility on the revision number
            if 'NACE_Rev' in href and 'Heading_All_Languages.tsv' in href:
                download_link = href
                logger.debug(f"Found EU SIC download link: {download_link}")
                break
        ...
        if not download_link:
            for link in soup.find_all('a', href=True):
                href = link['href']
                text = link.get_text().lower()
                if '.tsv' in href and ('nace' in href.lower() or 'nace' in text):
                    download_link = link['href']
                    logger.debug(f"Found TSV link with NACE: {download_link}")
                    break
        
        # Last resort - try to find any download section on the page
        if not download_link:
            # Log all links found on the page for debugging
            all_links = []
            for link in soup.find_all('a', href=True):
            ...
            # Try a more general approach to find download links
            for link in soup.find_all('a', href=True):
                href = link['href'].lower()
                text = link.get_text().lower()
                if any(term in href or term in text for term in ['download', 'tsv', 'csv', 'data']):
                    # This might be a download page, follow it
                    logger.info(f"Following potential download link: {link['href']}")
                    sub_url = urljoin(download_site, link['href'])
                    try:
                        sub_response = requests.get(sub_url)
                        sub_response.raise_for_status()
                        sub_soup = BeautifulSoup(sub_response.text, 'html.parser')
                        
                        # Look for the TSV file on this page
                        for sub_link in sub_soup.find_all('a', href=True):
                            sub_href = sub_link['href']
                            if 'NACE_Rev' in sub_href and 'Heading_All_Languages.tsv' in sub_href:
                                download_link = sub_href
                                logger.debug(f"Found EU SIC download link on subpage: {download_link}")
                                break
        ...
        # If still no link, use a hardcoded fallback as absolute last resort
        if not download_link:
            logger.warning("No download link found, using hardcoded fallback for NACE Rev 2.1")
            download_link = "https://showvoc.op.europa.eu/download/ESTAT_Statistical_Classification_of_Economic_Activities_in_the_European_Community_Rev._2.1._%28NACE_2.1%29/NACE_Rev2.1_Heading_All_Languages.tsv"
        ...
        # Download the file
        logger.info('Downloading EU SIC data from %s', download_link)
        file_response = requests.get(download_link)
        file_response.raise_for_status()
        ...
        # If filename doesn't have the expected pattern, create a proper one
        if not (filename.startswith('NACE_Rev') and filename.endswith('.tsv')):
            # Extract revision number if possible
            rev_match = re.search(r'Rev[_.]?(\d+\.?\d*)', download_link, re.IGNORECASE)
            rev_num = rev_match.group(1) if rev_match else '2.1'  # Default to Rev 2.1
            filename = f"NACE_Rev{rev_num}_Heading_All_Languages.tsv"
        ...
        logger.error('Error downloading EU SIC data: %s', str(e))
        return None
    except Exception as e:
        logger.error('Unexpected error downloading EU SIC data: %s', str(e))
        import traceback
        logger.error(traceback.format_exc())
        return None