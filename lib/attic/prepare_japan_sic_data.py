        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    elif hierarchy == '4' and code_full and current_major_group:
                        # Check if this is a group or industry group
                        if group_code and group_code != "nan":
                            ...
                            if groups:  # Make sure we have at least one group
                                last_group = groups[-1][0]
                                industry_groups.append([code_full, description, last_group, current_major_group, current_division])
                                logger.info(f'Added industry group: {code_full} - {description}')
                    ...
                    # Industry Group level (hierarchy=5)
                        ...
                        if groups:
                            ...
                            # logger.debug(f'Added industry group: {code_full} - {description}')
                            # Debug check for "oil" in description (case insensitive)
                            if 'oil' in description.lower():
