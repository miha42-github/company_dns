...
class WikipediaQueries:
    ...
    def get_firmographics(self, company_name):

        # Store data here for return to the caller
        firmographics = dict()

        company_page = wptools.page(company_name, silent=True)
        ...
        firmographics['country'] = self._get_item(company_info, ['location_country', 'hq_location_country'], r'[\[\]]', 0)
        firmographics['city'] = self._get_item(company_info, ['location_city', 'hq_location_city'], r'[\[\]]', 0)

        # Website
        firmographics['url'] = self._get_item(company_info, ['website', 'homepage', 'url'], r'[\{\}]', 1)
...
if "__name__" == '__main__':
    ...
    firmographics = query.get_firmographics(company_name)
